from ChicagoCrime_CAN import *
import queue

class ChicagoCrimeTest:
	'''
	Class to test ChicagoCrime Project
	'''
	...
	def __init__(self, test_dispatch_file="Chicago_Crimes_Test.csv"):
		...
		self.__chicago_crime_obj = ChicagoCrimeAnalysis("Chicago_Crimes_Test.csv")
	...
	def test_add_dispatch(self, test_decision=False, num_items=None):
		...
		for i in range(num_items):
			...
			self.__chicago_crime_obj.add_dispatch(i, dispatch)
			self.__test_queue.put((i, dispatch))
			print("++++")

		print("-----")
		print('Dispatch queue visualization')
		visualize_heap(self.__chicago_crime_obj.get_dispatch_queue(), 2)
		print("-----")

		print("If you see an error, you may not have a method called \"get_dispatch_queue\" to get your queue")
		if (not test_decision):
			for i in range(num_items):
				print("-----")
				print("You should see - " + str(self.__test_queue.get()))
				print("")
				return_dispatch = self.__chicago_crime_obj.get_dispatch_queue().extract()
				print("Your priority queue returned - " + str([return_dispatch.key, return_dispatch.value]))
				print("-----")
	...
	def test_decide_next_patrol(self, num_decisions=None):
		...
		print("First we'll see how your \"decide_next_patrol\" method does without having anything in the queue AND without any requests")
		...
		for i in range(num_decisions):
			...
			print("We're going to send the next patrol out to : " + self.__chicago_crime_obj.decide_next_patrol())
			print("-----")

		print("Now, let's add some stuff to your queue & see if we can use that to determine the next patrol (you should see some differences this time with the queue being available to help the decision)")
		...
		while (not self.__chicago_crime_obj.get_dispatch_queue().is_empty()):
			self.__chicago_crime_obj.get_dispatch_queue().extract()
	...
	def test_crime_priority_list(self):
		...
		print("If you used an object to encapsulate the payload (key & value), you'll need to override __repr__ or __str__ method for this to be helpful")

		# Go through list and print out each payload
		for i in range(10):#for i in range(len(self.__chicago_crime_obj.get_crime_priority_list())):
			print("-----")
			print("We probably should be at priority - " + str(i))
			print("This item has the following payload - " + str(self.__chicago_crime_obj.get_crime_priority_list()[i]))
			print("-----")
